...
for PERIOD in periodArr:
    dateObj = datetime.now() - timedelta(days=7)
    dateStartObj = sub_business_days(dateObj, PERIOD)
    ...
    df["Label"] = labels
    dateStart = str(dateDf[0]).split()[0]
    dateEnd = str(dateDf[len(dateDf)-1]).split()[0]
    ...
    periodSum += pred
    print(str(dateStart), str(dateEnd), " | ", STOCK, " | ", PERIOD, " | ", pred)
